...
def test_sqlite_logging_lifecycle():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert len(runs) == 1
        assert runs[0]["test_id"] == "TC-001"
        assert runs[0]["correctness"] == 0.8
        assert runs[0]["clinical_utility"] == 0.85
        assert runs[0]["question"] == "What is NSCLC protocol?"
